...
const BasketScreen = ({ navigation }) => {
  ...
  const removeProduct = async (id) => {
    ...
    await updateTotalPrice(filteredProducts).then(() => {
  ...
  const onIncreaseQuantity = async (id) => {
    ...
    product.count++;
    updateTotalPrice(shadowProducts).then((res) => {
  ...
  const onDecreaseQuantity = async (id) => {
    ...
    if (product.count > 1) {
      product.count--;
      updateTotalPrice(shadowProducts).then(() => {
        setProducts(shadowProducts);
      });
    }
  };
        ...
        >
          ...
          >
            ...
            <View style={{display: 'flex', flexDirection: 'column', justifyContent: 'flex-end'}}>
              ...
              >
                <Text
                  style={{ fontWeight: "bold", color: "#9fd236", fontSize: 17 }}
                >
